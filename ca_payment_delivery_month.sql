...
delivered_amount_currency as(
    select FORMAT_DATE("%b %Y", clean_order_status.event_date) as delivery_month,
    clean_order_status.event_date,
    ...
    where clean_order_status.event_date > currency.start_date and clean_order_status.status IN ('04_delivered', '02_authorized')
    ...
    sum(case when delivered_amount_currency.status = '04_delivered' then amount * eur_value else 0 end) as delivery_ca,
    sum(case when delivered_amount_currency.status = '02_authorized' then amount * eur_value else 0 end) as payment_ca,
